...
- `/api/sessions` — open/close/list sessions, fetch days for an archived session, and `GET /:id/activity-logs` (every `daily_activities` row for that session, open or closed: `{ sessionId, startDate, endDate, logs: [{ date, activity_id, amount }] }` — the activity charts' single read).
...
- `/api/activity-logs` — per-day activity totals for the open session (`daily_activities`). `GET ?date=` (default today) returns `{ date, logs: [{ activity_id, amount }] }`. `POST /` with `{ activity_id, amount, date? }` **adds** to that day's total (201 created / 200 incremented); `PUT /:date/:activityId` **replaces** it (201/200); `DELETE /:date/:activityId` clears it. Dates go through `validateSessionDate()`; every write 403s when the session is blocked. The Activities card only uses today; the charts read whole sessions via `/api/sessions/:id/activity-logs`.
...
On the client, the Home History table (`DayHistoryTable`) rows are clickable (via an opt-in `onSelectDay` prop) and open `DayDetailModal`, which shows that day's weight (log/edit/clear via `WeightEntry`), lists that day's meals with add/edit/delete (reusing `MealList` + `AddMealModal`, the latter taking an optional `date` prop), and ends with an Activities card (`ActivityList` for that date — add/set/clear any activity). Home also passes `weightsByDate` to the table so each past row shows its logged weight or "no weight", making missed days obvious. Both Home and `SessionDetail` pass `activitiesByDate` (the `byDate` map from `useActivitySeries`), which adds a full-width line of that day's activity totals under the bars (e.g. "Steps 8,432 · Push-ups 35 · Plank 90 sec" — the unit is omitted when it's `reps` or equals the activity name); days with no activities get no line. The same `DayHistoryTable` rendered read-only in `SessionDetail` omits `onSelectDay`, so archive rows are not clickable.
...
- Home section order is `SessionHeader` → `TodayTotals` → `MealList` → `WeightLogger` → `WeightChart` → `ActivityLogger` → `ActivityDailyChart` → `ActivityTotalsChart` → `DayHistoryTable`. `TodayTotals` renders the four-macro totals grid first, then today's calorie + protein progress bars below it (using the same `ProgressBar` as the history table, with `phase={session.phase}` on the calories bar); `DayHistoryTable` on Home is filtered to past days only (`d.date !== todayString()`), so today never appears in History. The archive view (`SessionDetail`) passes the unfiltered `days` so closed sessions still show every day, including the close day. `WeightLogger` is a thin card around `WeightEntry` (`components/WeightEntry.jsx`), the one log/edit/clear control for a given date; `DayDetailModal` reuses it for past days. `WeightChart` reads `useWeightHistory()` and renders a hand-rolled SVG (no chart-lib dependency) — area + line through the points, dashed goal line when `session.goal_weight_kg` is set, faint linear-regression trend line, hover/touch crosshair tooltip, and headline stats (current / from start / to goal / weekly trend). When `start_weight_kg` is set it is drawn as a hollow ring on day 0 and the "from start" delta is measured against it; if a weight was also logged on the start date, the start point is shifted to day −1 (the x-axis then begins there) so both points are kept and joined by the line. Empty and single-point states fall back to a short text message.
- `ActivityLogger` is the Activities card on Home: `ActivityList` for `todayString()`, plus a header toggle to `ActivityManager`. `ActivityList` (also used by `DayDetailModal` for past days) reads `useActivities()` + `useActivityLogs(date)` (both in `hooks/useActivities.js`) and renders one `ActivityEntry` per activity. `.activity-rows` is a CSS container: rows stack onto two lines when the list is narrower than 520px, so the day modal gets the stacked layout even on desktop. Each `ActivityEntry` row has an amount input with **Add** (POST, increments), **Set** (PUT, replaces) and **×** (DELETE). A header toggle swaps the list for `ActivityManager` (add/rename/delete custom activities, mirroring `CategoryManager`; presets are listed but not editable).
- The two activity charts take a `session` prop and render on both Home and `SessionDetail` (below the summary card). For a closed session with no activity logs (e.g. sessions from before activity tracking) they render nothing. They and share `useActivitySeries(session)` in `hooks/useActivities.js`, which joins `useActivityHistory(session.id)` with the catalog (so renames show up without refetching logs) and returns one `values[]` per activity logged at least once, over a day window that, while the session is open, spans at least 14 days (`MIN_WINDOW_DAYS`) so a new session isn't one full-width bar. Closed sessions span exactly their own days. It also returns `dayLabel(i)` / `endLabel`, which use "Today"/"Yesterday" only while the session is open, and `byDate` (`{ [date]: [{ activity, amount }] }`) for the History rows. `ActivityDailyChart` is small multiples — one bar strip per activity, each on its own scale (so steps sit next to reps), drawn as a `preserveAspectRatio="none"` SVG with one viewBox unit per day; hovering a day swaps every row's session total for that day's amount. `ActivityTotalsChart` plots cumulative session totals as lines. Only one unit is shown at a time (`reps` by default, with a switch when others are logged; units are grouped case-insensitively), chips toggle lines, and each line's colour is `--series-N` by the activity's position among catalog activities with that unit — so toggling never repaints. It measures its width with `hooks/useElementWidth.js` (ResizeObserver callback ref) and renders a 1:1 viewBox so axis text stays 11px on phones — unlike `WeightChart`'s fixed 600-wide viewBox. `--series-1..8` in `styles.css` is a fixed categorical order checked for colour-blind separation on `--surface`; series past 8 fall back to `--muted`.
...
| Add / set / clear activity    | `activityLogs.all` **and** `activities.list()` (refreshes `log_count`) — covers `activityLogs.list(sessionId, date)` and the charts' `activityLogs.history(sessionId)` |
