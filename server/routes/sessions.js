  ...
  const info = db
    .prepare(
      `INSERT INTO sessions
         (start_date, calorie_target, protein_target, status, start_weight_kg, goal_weight_kg, phase)
       VALUES (?, ?, ?, 'open', ?, ?, ?)`
    )
    .run(today(), Math.round(calories), Math.round(protein), startWeight, goalWeight, phaseValue);
